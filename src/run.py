...
def run(_run, _config, _log, pymongo_client=None):
    ...
    unique_token = "{}__{}__{}".format(args.env_args['map_name'],args.name, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        for _ in range(args.running_iters):
            episode_batch = runner.run(test_mode=False)
            buffer.insert_episode_batch(episode_batch)

        if buffer.can_sample(args.batch_size) and (args.full2train!=True or buffer.full()):
